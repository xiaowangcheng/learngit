...
public abstract class CommonRecyclerViewAdapter<T> extends RecyclerView.Adapter<CommonRecyclerViewHolder> {
    ...
    public void addFooterView(View view,boolean isNotify){
        if (view == null) {
            throw new NullPointerException("the header view can not be null");
        }
        foots.add(view);
        if(isNotify){
            notifyItemInserted(foots.size() - 1);
        }
    }
     ...
     * @param isNotify
     */
    public void removeFooterView(int position, boolean isNotify){
    ...
    public int getFooterCount(){return foots.size();}
    ...
    public CommonRecyclerViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        } else if (viewType == -2 && mCurrentPosition != -1 && mCurrentPosition >=(data.size()+foots.size())) { //说明是头部
            int fd  = data.size()+foots.size();
            int sdasd = mCurrentPosition;
            view = foots.get(mCurrentPosition-fd);
        } else {
    ...
    public int getItemViewType(int position) {
        ...
        if(position>=(data.size() + headers.size())){
            return -2;
        }
        if (position < size) {
    ...
    public int getItemCount() {
        return data.size() + getHeaderCounts()+getFooterCount();
    }
